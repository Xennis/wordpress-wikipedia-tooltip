jQuery.fn.mediaWikiTooltip = function(options) {
	var self = this;
	var location = this.prop('href').split('/');
	...
	var handlePage = function(page) {
		self.tooltipster({
			content: '<img src='+page.thumbnail.source+' style="float:right"><h5>'+page.title+'</h5><p style="text-align: justify">'+page.extract+' (Wikipedia)</p>',
			contentAsHTML: true,
